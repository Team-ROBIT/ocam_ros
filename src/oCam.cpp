...
public:
    ...
    Camera(int resolution, double frame_rate, const std::string& devPath): camera(NULL), devPath_(devPath) {
...
private:
    ...
    void device_poll() {
        ...
        image_transport::Publisher camera_image_pub = it.advertise("ocam1/camera/image_raw", 1);

        ros::Publisher camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("ocam1/camera/camera_info", 1);
...
public:
   ...
   * @param[in]  frame_rate  The frame rate
    ...
    oCamROS() {
        ...
        priv_nh.getParam("dev_path", devPath_);
        priv_nh.getParam("auto_exposure", autoexposure_);

        /* initialize the camera */
        ocam = new Camera(resolution_, frame_rate_, devPath_);
